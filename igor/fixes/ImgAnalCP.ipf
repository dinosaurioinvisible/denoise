...
// ferv edit: not deleting this, just in case
...
// not really necessary if panel's on the left
string info = StringByKey("SCREEN1", IgorInfo(0))
string info2 = replaceString(",", info, ";")
variable screenR = Str2num(StringFromList(3,info2))
variable screenB = Str2num(StringFromList(4,info2))

// just in case somebody wants to change this
variable panelW = 500
variable panelH = 500
...
If (WinType("IAControlPanel"))
	...
	// ferv edit: placing this to the right
	// variable bottom = top + 505
	variable top = 0
   variable left = screenR - panelW
	variable right = screenR
	variable bottom = top + panelH
	...
	// ferv edit
	NewPanel /K=2 /N=IAControlPanel /W=(left,top,right,bottom) /fg=(*,FR,*,*) as "Image Analysis"
	...
	strswitch(popstr) 
		...
		case "Display": 
			strswitch(g_NameOf3DWave)
				...
				break
				default:
				Display /k=1;AppendImage $g_NameOf3DWave
				DoUpdate
				SizeImage(300, WindowName=s_name);DelayUpdate
		...
		StrSwitch(PopStr)
			...
			case "Filter":
				...
				if(fil2 && g_AutoDisplay)
					...
					// ferv edit
					// display filD (prob someone deleted it by mistake)
					display $fild
					SizeImage(300)					
