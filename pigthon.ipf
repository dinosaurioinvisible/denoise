...
// 0
// we want to define paths tha can be reused
...
// for the paths to the script:
// we're creating also global variables, but no txts
// because you wouldn't want to auto load them for every new experiments
strConstant pigtonPythonPath = "none"
strConstant pathToScript = "none"
...
function runPythonScriptOnMovie(string path_to_movie, [string path_to_python_script, string path_to_python])
	...
	// TODO
	// fx to change script
	// check for fx argument
	if (paramIsDefault(path_to_python_script) == 0)
		svar pathToScript = path_to_python_script
	// if not, check for global variable (if script has run before)
	// else, call popup window
	elseif (cmpstr(pathToScript,"none") == 0)
		...
		svar pathToScript = s_fileName
	endif
	
	// 1.3 check path to movie
	if (paramIsDefault(path_to_movie) != 0)
		string message_movie = "select movie"
		Open/d/r/m=message_movie refNum
		path_to_movie = s_fileName
